/**
 * 批量修复 YAML front matter 缺少结束标记 --- 的问题（改进版）
 * 
 * 问题：某些文件的 YAML front matter 缺少结束标记 ---，
 * 导致 YAML 解析器把正文内容也当作 YAML 来解析，从而报错。
 * 
 * 修复方法：在第一个 markdown 标题（# 标题）之前，添加结束标记 ---。
 * 
 * 改进：只在第一 markdown 标题之前查找结束标记 ---，
 * 避免将正文中的 markdown 水平线（---）误认为是结束标记。
 */
...
console.log('开始修复 YAML front matter 缺少结束标记 --- 的问题（改进版）...');
console.log('');
