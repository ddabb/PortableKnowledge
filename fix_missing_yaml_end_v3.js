/**
 * 批量修复 YAML front matter 缺少结束标记 --- 的问题（最终版）
 * 
 * 改进：识别所有 markdown 标题（#、##、### 等），
 * 而不仅仅是 # 标题。
 */
...
function fixFile(filePath) {
  try {
    ...
    // 查找第一个 markdown 标题（任何级别）
    let firstMarkdownHeaderIndex = -1;
    for (let i = 1; i < lines.length; i++) {
      if (isMarkdownHeader(lines[i])) {
    ...
    // 在第一个 markdown 标题之前查找结束的 ---（第二个 ---）
    let endMarkerIndex = -1;
    for (let i = 1; i < firstMarkdownHeaderIndex; i++) {
      if (lines[i] === '---') {
        endMarkerIndex = i;
        break;
      }
    ...
    if (endMarkerIndex !== -1) {
      // 已经有结束标记（在第一个 markdown 标题之前），跳过
      skippedCount++;
      return;
    }
    
    // 没有结束标记，需要添加
    // 在第一个 markdown 标题之前插入结束标记 --- 和空行
...
console.log('开始修复 YAML front matter 缺少结束标记 --- 的问题（最终版）...');
console.log('');
